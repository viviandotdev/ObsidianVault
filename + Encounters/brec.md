...
def main():
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith('.epub'):
            ...
            # Extract author name from the end of the filename
            author_match = regex.search(r'\((.*?)\)\.epub$', clean_filename)

            if author_match:
                author = author_match.group(1)  # Reverse the matched string
                # Remove the author from clean_filename
                clean_filename = clean_filename.replace(f' ({author})', '')
            else:
                author = ""
            ...
            print(f"Processing '{clean_filename} {author}'...")
            try:
                ...
                # Send file with extracted author and cleaned title
                client.send_file(path_obj, ["08C185BFB09340EC8C022D18D9659F19"],
                                author=author,
